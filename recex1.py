...
import time
import traceback
import json # Añado la libreria para leer JSON
...
if not '--pyfepdf' in sys.argv:
    ...
    ENCABEZADO = [
        ...
        ('idioma_cbte', 1, A),
        ('cae', 14, N), 
        ('fecha_pago', 8, A), # RENZO. Modifico este valor (fecha_vto) por fecha_pago, para dar solución al error Err: 1672: Para facturas de exportacion (2  Servicios / 4 - Otros) es obligatorio informar la fecha de pago
        ('resultado', 1, A), 
        ('reproceso', 1, A),
        ('motivos_obs', 1000, A),
        ('id', 15, N),
...
def autorizar(ws, entrada, salida):
    ...
    if not DEBUG or not sys.stdout.isatty() or raw_input("Facturar?")=="S":
        ...
        'cae': dic['cae'],
        'obs': dic['motivos_obs'].encode("ascii", "ignore"),
        'err': dic['err_msg'].encode("ascii", "ignore"),
        'reproceso': dic['reproceso'],
        'cbte_nro': dic['cbte_nro']})
...
if __name__ == "__main__":
    ...
    try:
        ...
        if '/cae' in sys.argv:
            # Leo el archivo json que contine los datos de la factura
            data = []
            ...
            for factura in data:                
                ...
                cbte_nro = int(ws.GetLastCMP(tipo_cbte, punto_vta)) + 1
                fecha_cbte = datetime.datetime.now().strftime("%Y%m%d")
                tipo_expo = factura['tipo_expo']
                permiso_existente = factura['permiso_existente']
                dst_cmp = factura['pais_dst_cmp']
                cliente = factura['nombre_cliente']
                cuit_pais_cliente = factura['nro_doc']
                domicilio_cliente = factura['domicilio_cliente']
                id_impositivo = factura['id_impositivo']
                moneda_id = factura['moneda_id']
                moneda_ctz = factura['moneda_ctz']
                obs_comerciales = factura['obs_comerciales']
                obs = factura['obs_generales']
                forma_pago = factura['forma_pago']
                incoterms = factura['incoterms']
                incoterms_ds = factura['incoterms_ds'] 
                idioma_cbte = factura['idioma_cbte']
                ...
                # Agrego un item:
                codigo = factura['detalles']['codigo']
                ds = factura['detalles']['ds']
                qty = factura['detalles']['qty']
                precio = factura['detalles']['precio']
                umed = factura['detalles']['umed']
                bonif = factura['detalles']['bonif']
                imp_total = factura['detalles']['importe']
                # lo agrego a la factura (internamente, no se llama al WebService):
                ok = ws.AgregarItem(codigo, ds, qty, umed, precio, imp_total, bonif)

                # Agrego un permiso (ver manual para el desarrollador)
                #id = factura['permisos']['id_permiso']
                #dst = factura['permisos']['dst_merc']
                #ok = ws.AgregarPermiso(id, dst)
